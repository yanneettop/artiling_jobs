...
export function Dashboard(){
 ...
 const leads=db.leads.filter(l=>!l.archived_at)
 const attention=leads.filter(l=>ACTIVE_LEAD_STATUSES.includes(l.status)&&(l.priority==='HIGH'||(l.next_action_due&&l.next_action_due<=today))).slice(0,5)
 ...
 const metrics=[
  ['New Leads',leads.filter(l=>l.status==='NEW').length,'Awaiting first response','/leads?filter=NEW','neutral'],
  ['Needs Attention',attention.length,'Overdue or high priority','/leads?filter=attention','alert'],
  ['Ready to Quote',leads.filter(l=>l.status==='READY_TO_QUOTE').length,'Pricing complete','/leads?filter=READY_TO_QUOTE','warm'],
 ...
 return <>
  ...
  <div className="dashboard-grid">
   ...
   <Panel title="Pipeline" subtitle="Live leads by commercial stage">
    <div className="pipeline">{pipeline.map((status,i)=>{const count=leads.filter(l=>l.status===status).length;return <a href={`#/leads?filter=${status}`} key={status}><span className="pipeline-order">0{i+1}</span><div><strong>{label(status)}</strong><small>{count} {count===1?'lead':'leads'}</small></div><b>{count}</b></a>})}</div>
   </Panel>
