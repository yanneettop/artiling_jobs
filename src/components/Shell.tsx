...
export function Shell({path,children,onLogout}:{path:string;children:ReactNode;onLogout:()=>void}){
 ...
 return <div className="app-shell">
   ...
   <aside className={`sidebar ${mobileOpen?'open':''}`}>
     <div className="brand"><img className="brand-logo" src="/artiling-bimi-final.svg?v=white" alt="Artiling Studio"/><div><strong>ARTILING</strong><span>JOBS</span></div><button className="mobile-close" onClick={()=>setMobileOpen(false)} aria-label="Close navigation"><X size={20}/></button></div>
